...
**① 레이어 — raw source / wiki /schema +  infra**

| 레이어            | 경로          | 역할                                                                 |
| -------------- | ----------- | ------------------------------------------------------------------ |
| **raw source** | `raw/`      | 진실의 원천 — 사용자가 쓰는 노트. **불변(읽기 전용)**: wiki가 틀리면 원본이 아니라 wiki를 다시 만든다 |
| **wiki**       | `wiki/`     | LLM 생성물 — 정제·연결된 지식 페이지                                            |
| **schema**     | `AGENTS.md` | wiki를 어떻게 생성·병합·유지할지 규칙(단일 진실 공급원)                                 |
| **infra**      | `infra/`    | 옵시디언 설정·산출물 (LLM Wiki 의 구성요소는 아님)                                  |
...
- **revert(되돌리기)**: 잘못 편입되거나 원본이 이동·삭제됐을 때, 그 소스의 기여분만 언머지 (LLM Wiki 의 기본 오퍼레이션은 아님)

**⑤ 품질 장치 — 신뢰성을 지키는 규칙**
- **append-only 병합**(덮어쓰기 금지)
- **verbatim 인용**(출처를 원문 그대로 — 되돌리기·검증의 근거) · **모순 보존**(소스 간 충돌은 숨기지 않고 `## 모순`에 양측 보존) · **contentHash**(본문 안 바뀌면 재수집 시 즉시 skip)
