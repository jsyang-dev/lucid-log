...
각 오퍼레이션을 **스킬(어떻게 — 절차·규칙) + 에이전트(누가 — 실행 주체)** 로 분리했다. 반복·재사용 가능하고, 다음 세션에서도 description 키워드로 자동 트리거된다(=하네스로 구현).
...
- **질의**: "물류 시스템의 가용재고 어떻게 도나" → wiki 근거로 답 + `출처 페이지` 명시. 없으면 "wiki에 없음"이라고 정직하게.
