...
---
type: concept
sources: ["sources/흥진고-1학년-정보-정리"]
tags: method
aliases: [버블정렬, Bubble Sort]
---
### 정의
버블정렬은 인접한 두 데이터를 비교해 순서가 어긋나면 자리를 교환하는 과정을 반복해 정렬하는 알고리즘이다.

### 주요 특징
- 인접한 두 원소 비교 후 교환(`a[j] > a[j+1]`이면 swap)
- 이중 반복: 바깥 `n-1`, 안쪽 `n-i-1`

### 관련 엔티티
- Python

### 출처 언급
- "인접한 두 개의 데이터를 비교하여 자리를 교환하는 방법" — 흥진고 1학년 정보 정리
```
