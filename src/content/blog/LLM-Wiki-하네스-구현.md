...
**트레이드오프**: 합성이 수집 시점에 baked-in되므로 **수집 시점의 오독이 그 페이지에서 도출되는 모든 답변에 내장**된다(RAG는 매 쿼리 원본을 다시 읽는다). 따라서 `lint`로 모순·stale을 검출하고, 필요 시 `revert` 후 **재수집**으로 페이지를 다시 세우는 유지 루프가 중요하다.
